<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-xl font-semibold leading-tight text-novix-ink dark:text-white">Medications</h2>
                <p class="mt-1 text-sm text-novix-muted">For {{ $active->full_name }}</p>
            </div>
            @if($canEdit)
                <a href="{{ route('medications.create') }}?member={{ $active->id }}" class="flex items-center gap-2 rounded-xl bg-novix-green px-5 py-2.5 text-sm font-semibold text-white shadow-novix-sm transition hover:bg-novix-green-dark">
    ...
    <div class="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        @if($medications->isEmpty())
            <div class="rounded-novix bg-white p-10 text-center shadow-novix-sm dark:bg-white/5">
                ...
                <p class="mt-3 text-sm text-novix-muted">No medications recorded yet.</p>
                @if($canEdit)
                    <a href="{{ route('medications.create') }}?member={{ $active->id }}" class="mt-2 inline-block text-xs font-semibold text-novix-green hover:underline">Add the first one</a>
                @endif
            </div>
        @else
            <div class="space-y-3">
                        ...
                        <div class="flex items-start justify-between gap-3">
                            <div class="min-w-0">
                                <div class="flex flex-wrap items-center gap-2">
                                    <p class="text-sm font-bold text-novix-ink dark:text-white">{{ $medication->medicine_name }}</p>
                                    ...
                                    @if($medication->reminder_enabled)
                                        <span class="flex items-center gap-1 rounded-full bg-novix-mint px-2 py-0.5 text-[10px] font-bold text-novix-green dark:bg-novix-green/20 dark:text-novix-mint" title="Reminders on">
                                ...
                                @if(!empty($medication->schedule_times))
                                    <div class="mt-2 flex flex-wrap gap-1.5">
                                        @foreach($medication->schedule_times as $time)
                                            <span class="rounded-full bg-novix-cream px-2 py-0.5 text-[11px] font-semibold text-novix-ink dark:bg-white/10 dark:text-white">{{ $time }}</span>
                                        @endforeach
                                    </div>
                                @endif
                                @if($medication->start_date || $medication->end_date)
                                    <p class="mt-2 text-[11px] text-novix-muted">
                            ...
                            @if($canEdit)
                                <div class="flex flex-shrink-0 items-start gap-1">
                                    <a href="{{ route('medications.edit', $medication) }}" class="rounded-lg px-3 py-2 text-xs font-semibold text-novix-green hover:bg-novix-mint/40 dark:hover:bg-white/10">Edit</a>
