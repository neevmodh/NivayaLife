<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                <h2 class="text-xl font-semibold leading-tight text-novix-ink dark:text-white">Failed jobs</h2>
    ...
    <div class="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        @else
            ...
            <div class="space-y-3">
                @foreach($jobs as $job)
                    <div class="rounded-novix border-t-2 border-novix-pink-dark/50 bg-white p-4 shadow-novix-sm dark:bg-white/5" x-data="{ open: false }">
                        <div class="flex flex-wrap items-start justify-between gap-3">
                            <div class="min-w-0 flex-1">
                                <p class="truncate text-sm font-bold text-novix-ink dark:text-white">{{ $job->name }}</p>
                                ...
                                <p class="mt-1.5 break-words text-xs text-novix-pink-dark">{{ Str::limit($job->reason, 200) }}</p>
                            </div>

                            <div class="flex flex-shrink-0 gap-2">
                                <form method="POST" action="{{ route('admin.failed-jobs.retry', $job->uuid) }}">
                                    @csrf
                                    <button type="submit" class="rounded-lg bg-novix-green px-3 py-1.5 text-xs font-bold text-white transition hover:bg-novix-green-dark active:scale-95">Retry</button>
                                </form>
                                <form method="POST" action="{{ route('admin.failed-jobs.delete', $job->uuid) }}"
                                    ...
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="rounded-lg border border-gray-200 px-3 py-1.5 text-xs font-bold text-novix-muted transition hover:border-novix-pink-dark hover:text-novix-pink-dark active:scale-95 dark:border-white/10">Discard</button>
                                </form>
                            </div>
                        </div>

                        <button type="button" @click="open = !open" class="mt-2 text-[11px] font-semibold text-novix-muted transition hover:text-novix-green" :aria-expanded="open.toString()">
                            <span x-text="open ? 'Hide stack trace' : 'Show stack trace'"></span>
                        </button>
                        <pre x-show="open" x-cloak class="mt-2 max-h-64 overflow-auto rounded-lg bg-novix-cream/70 p-3 text-[10px] leading-relaxed text-novix-ink dark:bg-black/30 dark:text-white/70">{{ $job->exception }}</pre>
                    </div>
                @endforeach
            </div>
        @endif
