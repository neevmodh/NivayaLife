<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                <h2 class="text-xl font-semibold leading-tight text-novix-ink dark:text-white">Users</h2>
    ...
    <div class="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        @if($users->isEmpty())
            <div class="rounded-novix bg-white p-10 text-center shadow-novix-sm dark:bg-white/5">
                <p class="text-sm text-novix-muted">No users match that search.</p>
            </div>
        @else
            <div class="space-y-2">
                @foreach($users as $user)
                    <a href="{{ route('admin.users.show', $user) }}"
                        class="novix-gold-edge flex items-center gap-4 rounded-novix bg-white p-4 shadow-novix-sm transition hover:-translate-y-0.5 hover:shadow-novix dark:bg-white/5">
                        <x-avatar :photo-path="$user->avatar_path" :full-name="$user->name" size="h-10 w-10" />
                        <div class="min-w-0 flex-1">
                            <div class="flex flex-wrap items-center gap-2">
                                <p class="truncate text-sm font-bold text-novix-ink dark:text-white">{{ $user->name }}</p>
                                @if($user->is_admin)
                                    <span class="rounded-full bg-novix-gold/20 px-2 py-0.5 text-[10px] font-bold text-amber-700 dark:text-novix-gold">Admin</span>
                                @endif
                                @unless($user->email_verified_at)
                                    <span class="rounded-full bg-novix-yellow/30 px-2 py-0.5 text-[10px] font-bold text-amber-700 dark:text-novix-yellow">Unverified</span>
                                @endunless
                            </div>
                            <p class="truncate text-xs text-novix-muted">{{ $user->email }}</p>
                        </div>
                        <div class="flex-shrink-0 text-right">
                            <p class="text-sm font-bold text-novix-ink dark:text-white">{{ $user->family_members_count }}</p>
                            <p class="text-[11px] text-novix-muted">members</p>
                        </div>
                        <svg class="h-4 w-4 flex-shrink-0 text-novix-muted" viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M9 6l6 6-6 6" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
                    </a>
                @endforeach
            </div>
