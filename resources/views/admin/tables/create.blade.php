<x-app-layout>
    ...
    <div class="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <form method="POST" action="{{ route('admin.tables.store', $table) }}" class="space-y-4 rounded-nivayalife bg-white p-6 shadow-nivayalife-sm dark:bg-white/5">
            @csrf
            @include('admin.tables._form-fields', ['columns' => $columns, 'values' => null])

            <div class="flex justify-end pt-2">
                <button type="submit" class="rounded-xl bg-nivayalife-green px-6 py-2.5 text-sm font-semibold text-white shadow-nivayalife-sm hover:bg-nivayalife-green-dark">Create row</button>
            </div>
        </form>
    </div>
</x-app-layout>
