@php
    $statusBadge = [
        'active' => ['label' => 'Active', 'class' => 'bg-nivayalife-mint text-nivayalife-green dark:bg-nivayalife-green/20 dark:text-nivayalife-mint'],
        'expired' => ['label' => 'Expired', 'class' => 'bg-gray-100 text-gray-500 dark:bg-white/10 dark:text-white/60'],
        'revoked' => ['label' => 'Revoked', 'class' => 'bg-nivayalife-pink/30 text-nivayalife-pink-dark'],
        'viewed' => ['label' => 'Viewed (one-time)', 'class' => 'bg-nivayalife-yellow/30 text-nivayalife-yellow'],
    ];
@endphp
    ...
    <div class="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        @if($shares->isEmpty())
            <div class="rounded-nivayalife bg-white p-10 text-center shadow-nivayalife-sm dark:bg-white/5">
                <p class="text-sm text-nivayalife-muted">No shares created yet.</p>
            </div>
        @else
            <div class="space-y-3">
                @foreach($shares as $share)
                    @php($badge = $statusBadge[$share->statusLabel()])
                    <div class="rounded-nivayalife bg-white p-4 shadow-nivayalife-sm dark:bg-white/5">
                        <div class="flex flex-wrap items-start justify-between gap-3">
                            <div class="min-w-0">
                                <div class="flex items-center gap-2">
                                    <p class="text-sm font-bold text-nivayalife-ink dark:text-white">
                            ...
                            @if($share->isActive())
                                <form method="POST" action="{{ route('shares.revoke', $share) }}" onsubmit="return confirm('Revoke this share? The link will stop working immediately.');">
                                    @csrf
                                    <button type="submit" class="flex-shrink-0 rounded-lg border border-nivayalife-pink-dark/30 px-3 py-1.5 text-xs font-semibold text-nivayalife-pink-dark hover:bg-nivayalife-pink/10">Revoke</button>
                                </form>
                            @endif
                        </div>
                    </div>
