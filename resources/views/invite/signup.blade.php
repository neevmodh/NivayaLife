...
<div class="mx-auto max-w-xl px-4 py-10 sm:py-14">
    ...
    <div
        ...
        class="rounded-nivayalife bg-white p-6 shadow-nivayalife sm:p-8"
    >
        ...
        <h1 class="mt-4 text-2xl font-bold text-nivayalife-ink">Join {{ $invitation->primaryAccount->name }}'s family on Nivaya Life</h1>
        <p class="mt-1 text-sm text-nivayalife-muted">Create your own login — you'll choose exactly what to share.</p>

        <form @submit.prevent="submit($event)" class="mt-6 space-y-4">
            <input type="hidden" name="email" value="{{ $invitation->invited_email }}">
            <div class="rounded-xl bg-nivayalife-cream/60 px-4 py-2.5 text-sm text-nivayalife-ink">{{ $invitation->invited_email }}</div>
            ...
            <div>
                <p class="mb-2 text-sm font-semibold text-nivayalife-ink">Your photo</p>
                <x-camera-capture :upload-url="'#'" element-id="nivayalife-invite-camera" />
