...
<x-app-layout>
    ...
    <div class="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8" x-data="{ tab: @js($initialTab) }">
        ...
        <div class="relative" x-data="{ tabsOverflow: false }" x-init="$nextTick(() => tabsOverflow = $refs.tabScroll.scrollWidth > $refs.tabScroll.clientWidth + 4)">
            ...
            <div x-show="tabsOverflow" x-cloak class="pointer-events-none absolute inset-y-0 right-0 w-10 rounded-r-full bg-gradient-to-l from-nivayalife-mint to-transparent dark:from-white/10"></div>
        </div>

        <div class="mt-6 rounded-nivayalife bg-white p-6 shadow-nivayalife-sm sm:p-8 dark:bg-white/5">
