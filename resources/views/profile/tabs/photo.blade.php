...
<div>
    <h3 class="text-lg font-bold text-nivayalife-ink dark:text-white">Photo</h3>
    ...
    <div class="mt-6" x-data="{ retaking: false }">
        <template x-if="!retaking">
            <div class="flex flex-col items-center gap-4">
                <x-avatar :photo-path="$member->photo_path" :preset="$member->avatar_preset ?? null" :full-name="$member->full_name" :gender="$member->gender" :age="$member->age()" size="h-40 w-40" class="shadow-nivayalife-sm" />
                <button type="button" @click="retaking = true" class="rounded-xl bg-nivayalife-green px-6 py-2.5 text-sm font-semibold text-white shadow-nivayalife-sm hover:bg-nivayalife-green-dark">
        ...
        <template x-if="retaking">
            <div x-data="{ done: false }">
                <div x-show="!done">
                    <x-camera-capture :upload-url="$isDependentEdit ? route('profile.photo.member', $member) : route('profile.photo')" :existing-preview-url="$currentPhotoUrl" element-id="nivayalife-profile-camera" />
                    <div class="mt-5 flex justify-center">
                        <button type="button" @click="
                            const cam = window.Alpine.$data(document.getElementById('nivayalife-profile-camera'));
                            cam.nivayaLifeUpload().then(ok => { if (ok) { done = true; setTimeout(() => window.location.reload(), 1200); } });
                        " class="rounded-xl bg-nivayalife-green px-6 py-2.5 text-sm font-semibold text-white shadow-nivayalife-sm hover:bg-nivayalife-green-dark">
                            Save photo
                        </button>
                    </div>
                </div>
                <div x-show="done" x-cloak class="flex flex-col items-center gap-2 py-8 text-nivayalife-green">
    ...
    <div class="mt-10 border-t border-gray-100 pt-8 dark:border-white/10"
        ...
        <div class="flex items-center justify-between gap-3">
            <div>
                <h3 class="text-lg font-bold text-nivayalife-ink dark:text-white">Or pick an avatar</h3>
                <p class="mt-1 text-sm text-nivayalife-muted">
            ...
            <span x-cloak x-show="saved" class="flex-shrink-0 rounded-full bg-nivayalife-mint px-3 py-1 text-xs font-bold text-nivayalife-green dark:bg-nivayalife-green/20 dark:text-nivayalife-mint">Saved</span>
        </div>
        ...
        @foreach(\App\Support\AvatarPresets::grouped() as $group => $presets)
            <div class="mt-5">
                <p class="mb-2 text-[11px] font-bold uppercase tracking-wider text-nivayalife-muted">{{ $group }}</p>
                <div class="grid grid-cols-5 gap-3 sm:grid-cols-8">
                    @foreach($presets as $key => $data)
                        <button type="button" @click="choose(@js($key))" :disabled="saving"
                            ...
                            :class="selected === @js($key) ? 'ring-2 ring-nivayalife-green ring-offset-2 dark:ring-offset-nivayalife-night' : 'ring-1 ring-gray-200 hover:ring-nivayalife-green/40 dark:ring-white/10'"
                            ...
                            <span x-cloak x-show="selected === @js($key)"
                                class="absolute -bottom-0.5 -right-0.5 flex h-4 w-4 items-center justify-center rounded-full bg-nivayalife-green text-white ring-2 ring-white dark:ring-nivayalife-night" aria-hidden="true">
