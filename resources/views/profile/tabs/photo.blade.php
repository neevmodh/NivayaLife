...
<div>
    ...
    <p class="mt-1 text-sm text-novix-muted">Used on your emergency ID card — retake or upload a new one any time.</p>
    ...
    <div class="mt-6" x-data="{ retaking: false }">
        <template x-if="!retaking">
            <div class="flex flex-col items-center gap-4">
                ...
                <button type="button" @click="retaking = true" class="rounded-xl bg-novix-green px-6 py-2.5 text-sm font-semibold text-white shadow-novix-sm hover:bg-novix-green-dark">
                    Retake selfie / upload new photo
                </button>
            </div>
        </template>
        ...
        <template x-if="retaking">
            <div x-data="{ done: false }">
                <div x-show="!done">
                    <x-camera-capture :upload-url="route('profile.photo')" :existing-preview-url="$currentPhotoUrl" element-id="novix-profile-camera" />
