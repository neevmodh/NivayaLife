...
<div x-data="ajaxForm({ url: @js($isDependentEdit ? route('profile.health.member', $member) : route('profile.health')), csrfToken: @js(csrf_token()) })">
    <h3 class="text-lg font-bold text-nivayalife-ink dark:text-white">Health</h3>
    <p class="mt-1 text-sm text-nivayalife-muted">Saving a new height/weight adds a fresh entry to {{ $isDependentEdit ? 'their' : 'your' }} BMI history rather than overwriting the last one.</p>
