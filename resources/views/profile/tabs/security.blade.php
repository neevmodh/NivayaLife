<div class="space-y-10">
    <div>
        <h3 class="text-lg font-bold text-nivayalife-ink dark:text-white">Change password</h3>
        ...
        <form method="POST" action="{{ route('password.update') }}" class="mt-6 space-y-4">
            ...
            <div class="flex items-center gap-3">
                <button type="submit" class="rounded-xl bg-nivayalife-green px-6 py-2.5 text-sm font-semibold text-white shadow-nivayalife-sm hover:bg-nivayalife-green-dark">
                    Update password
                </button>
                @if(session('status') === 'password-updated')
                    <span class="flex items-center gap-1.5 text-sm font-semibold text-nivayalife-green">
    ...
    @if($archivedMembers->isNotEmpty())
        <div class="border-t border-gray-100 pt-8 dark:border-white/10">
            <h3 class="text-lg font-bold text-nivayalife-ink dark:text-white">Archived family members</h3>
            <p class="mt-1 text-sm text-nivayalife-muted">Their medical history was kept, not deleted. Restore anytime.</p>
                ...
                @foreach($archivedMembers as $archived)
                    <div class="flex items-center justify-between rounded-xl border border-gray-100 px-4 py-3 dark:border-white/10">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <p class="text-sm font-medium text-nivayalife-ink dark:text-white">{{ $archived->full_name }}</p>
                                <p class="text-xs text-nivayalife-muted">Archived {{ $archived->deleted_at->diffForHumans() }}</p>
                            </div>
                        </div>
                        <form method="POST" action="{{ route('family.restore', $archived->id) }}">
                            @csrf
                            <button type="submit" class="rounded-lg border border-nivayalife-green/30 px-3 py-1.5 text-xs font-semibold text-nivayalife-green hover:bg-nivayalife-mint/40">Restore</button>
                        </form>
                    </div>
                @endforeach
            </div>
    ...
    <div class="border-t border-gray-100 pt-8 dark:border-white/10">
        <h3 class="text-lg font-bold text-nivayalife-ink dark:text-white">Linked accounts</h3>
        <div class="mt-4 flex items-center justify-between rounded-xl border border-gray-100 px-4 py-3 dark:border-white/10">
            <div class="flex items-center gap-3">
                ...
                <span class="text-sm font-medium text-nivayalife-ink dark:text-white">Google</span>
            </div>
            @if($user->google_id)
                <span class="rounded-full bg-nivayalife-mint px-3 py-1 text-xs font-semibold text-nivayalife-green">Connected</span>
            @else
    ...
    <div class="border-t border-nivayalife-pink-dark/20 pt-8" x-data="{ confirming: false, typed: '' }">
        <h3 class="text-lg font-bold text-nivayalife-pink-dark">Danger zone</h3>
        <p class="mt-1 text-sm text-nivayalife-muted">Deleting your account immediately removes your profile, reports, and data from active use, and signs you out everywhere. A record is retained internally for account recovery and audit purposes. If you sign up again with this email later, it starts as a completely new account — this data is not restored to it.</p>

        <button type="button" x-show="!confirming" @click="confirming = true"
            class="mt-4 rounded-xl border border-nivayalife-pink-dark px-6 py-2.5 text-sm font-semibold text-nivayalife-pink-dark hover:bg-nivayalife-pink-dark hover:text-white">
            Delete my account and all data
        </button>

        <form x-show="confirming" x-cloak method="POST" action="{{ route('profile.destroy') }}" class="mt-4 space-y-4 rounded-xl border border-nivayalife-pink-dark/30 bg-nivayalife-pink/5 p-5">
            @csrf
            @method('delete')
            ...
            @else
                <p class="text-xs text-nivayalife-muted">You sign in via Google only, so there's no password to confirm — typing DELETE below is enough.</p>
            @endif

            <div>
                <label class="mb-1.5 block text-xs font-semibold text-nivayalife-muted">Type <strong>DELETE</strong> to confirm</label>
                <input type="text" name="confirmation" x-model="typed"
                    class="w-full rounded-xl border border-nivayalife-pink-dark/40 bg-white px-4 py-3 text-sm focus:border-nivayalife-pink-dark focus:outline-none focus:ring-2 focus:ring-nivayalife-pink-dark/20">
            </div>

            <div class="flex items-center gap-3">
                <button type="submit" :disabled="typed !== 'DELETE'"
                    class="rounded-xl bg-nivayalife-pink-dark px-6 py-2.5 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-40">
                    Permanently delete account
                </button>
                <button type="button" @click="confirming = false" class="text-sm font-semibold text-nivayalife-muted hover:text-nivayalife-ink">Cancel</button>
            </div>
        </form>
    </div>
</div>
