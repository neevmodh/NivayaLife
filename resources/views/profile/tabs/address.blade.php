<div x-data="ajaxForm({ url: @js($isDependentEdit ? route('profile.address.member', $member) : route('profile.address')), csrfToken: @js(csrf_token()) })">
    <h3 class="text-lg font-bold text-nivayalife-ink dark:text-white">Address</h3>
    <p class="mt-1 text-sm text-nivayalife-muted">Kept on file for {{ $isDependentEdit ? 'their' : 'your' }} records and, if ever needed, emergency responders.</p>
