...
<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                <h2 class="text-2xl font-bold leading-tight tracking-tight text-nivayalife-ink dark:text-white">Reports</h2>
                <p class="mt-1 text-sm text-nivayalife-muted">{{ $active->full_name }}</p>
            </div>
            <div class="flex items-center gap-2">
                <a href="{{ route('shares.history') }}?member={{ $active->id }}"
                    class="rounded-xl border border-gray-200 px-4 py-2.5 text-sm font-semibold text-nivayalife-ink transition hover:-translate-y-0.5 hover:bg-nivayalife-cream active:translate-y-0 dark:border-white/10 dark:text-white dark:hover:bg-white/10">
                    Share history
                </a>
        ...
        @if($reports->isNotEmpty() || $hasFilters)
            <div class="mb-4 grid grid-cols-3 gap-3">
                <div class="rounded-nivayalife bg-white p-3.5 text-center shadow-nivayalife-sm dark:bg-white/5">
                    <p class="text-xl font-extrabold text-nivayalife-ink dark:text-white">{{ $reports->count() }}</p>
                    <p class="text-[11px] font-semibold text-nivayalife-muted">{{ $hasFilters ? 'Matching' : 'Total' }}</p>
                </div>
                <div class="rounded-nivayalife bg-white p-3.5 text-center shadow-nivayalife-sm dark:bg-white/5">
                    <p class="text-xl font-extrabold text-nivayalife-green dark:text-nivayalife-mint">{{ $readyCount }}</p>
                    <p class="text-[11px] font-semibold text-nivayalife-muted">Explained</p>
                </div>
                <div class="rounded-nivayalife bg-white p-3.5 text-center shadow-nivayalife-sm dark:bg-white/5">
                    <p class="text-xl font-extrabold {{ $pendingCount > 0 ? 'text-amber-600 dark:text-nivayalife-yellow' : 'text-nivayalife-ink dark:text-white' }}">{{ $pendingCount }}</p>
                    <p class="text-[11px] font-semibold text-nivayalife-muted">Processing</p>
                </div>
            </div>
        @endif

        {{-- Search stays visible; the rest folds away until wanted --}}
        <form method="GET" action="{{ route('reports.index') }}" class="mb-4 rounded-nivayalife bg-white p-3 shadow-nivayalife-sm dark:bg-white/5">
            @if(request('member'))
            ...
            <div class="flex items-center gap-2">
                ...
                <button type="button" @click="filtersOpen = !filtersOpen"
                    class="flex items-center gap-1.5 rounded-xl border border-gray-200 px-3 py-2.5 text-sm font-semibold text-nivayalife-ink transition hover:bg-nivayalife-cream active:scale-95 dark:border-white/10 dark:text-white dark:hover:bg-white/10"
                    ...
                    @if(count($activeFilters))
                        <span class="flex h-4 w-4 items-center justify-center rounded-full bg-nivayalife-gold text-[10px] font-bold text-white">{{ count($activeFilters) }}</span>
                    @endif
                </button>
            ...
            <div x-show="filtersOpen" x-cloak x-transition class="mt-3 grid grid-cols-1 gap-3 border-t border-gray-100 pt-3 dark:border-white/10 sm:grid-cols-3">
                <div>
                    <label class="mb-1 block text-xs font-semibold text-nivayalife-muted">Type</label>
            ...
            @if(count($activeFilters))
                <div class="mt-3 flex flex-wrap items-center gap-2 border-t border-gray-100 pt-3 dark:border-white/10">
                    @foreach($activeFilters as $filterLabel)
                        <span class="inline-flex items-center rounded-full bg-nivayalife-mint px-2.5 py-1 text-[11px] font-semibold text-nivayalife-green dark:bg-nivayalife-green/20 dark:text-nivayalife-mint">
                            {{ $filterLabel }}
                        </span>
                    @endforeach
                    <a href="{{ route('reports.index') }}{{ request('member') ? '?member='.request('member') : '' }}"
                        class="text-[11px] font-semibold text-nivayalife-muted transition hover:text-nivayalife-pink-dark">Clear all</a>
                </div>
            @endif
        </form>
        ...
        @else
            @foreach($grouped as $month => $monthReports)
                <div class="mb-2 mt-6 flex items-center gap-3 first:mt-0">
                    <h3 class="text-[11px] font-bold uppercase tracking-wider text-nivayalife-muted">{{ $month }}</h3>
                    <span class="h-px flex-1 bg-gray-100 dark:bg-white/10" aria-hidden="true"></span>
                    <span class="text-[11px] font-semibold text-nivayalife-muted">{{ $monthReports->count() }}</span>
                </div>
                        ...
                        <a href="{{ route('reports.show', $report) }}"
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                @if($report->ai_summary)
                                    <p class="mt-1.5 line-clamp-2 text-xs leading-relaxed text-nivayalife-muted">{{ Str::limit(Str::of($report->ai_summary)->before("\n\n"), 150) }}</p>
                                @endif
                            </div>
                            <svg class="h-4 w-4 flex-shrink-0 self-center text-nivayalife-muted transition group-hover:translate-x-0.5 group-hover:text-nivayalife-green" viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M9 6l6 6-6 6" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
                        </a>
                    @endforeach
                </div>
            @endforeach
