...
<x-app-layout>
    <x-slot name="header">
        <h2 class="text-2xl font-bold leading-tight tracking-tight text-nivayalife-ink dark:text-white">{{ $member->full_name }}</h2>
    </x-slot>

    <div class="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
        <a href="{{ route('family.index') }}" class="mb-4 inline-flex items-center gap-1 text-sm font-semibold text-nivayalife-green hover:underline">
        ...
        @if(!$canEdit)
            <div class="mt-6 rounded-nivayalife bg-white p-6 text-center shadow-nivayalife-sm dark:bg-white/5">
                <span class="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-nivayalife-cream text-nivayalife-muted dark:bg-white/10">
                ...
                @if($activeGrant)
                    <p class="mt-3 text-sm text-nivayalife-ink dark:text-white">{{ $member->full_name }} has shared <strong>{{ Str::headline($activeGrant->scope) }}</strong> access with you — full profile details aren't visible from here.</p>
                @else
                    <p class="mt-3 text-sm text-nivayalife-ink dark:text-white">{{ $member->full_name }} hasn't granted you access to their profile yet.</p>
                    <p class="mt-1 text-xs text-nivayalife-muted">Only they can choose to share their data — this is their decision to make from their own account.</p>
                @endif
            </div>
        @else
            @if($member->access_type === 'linked')
                <div class="mt-6 flex items-center gap-2 rounded-nivayalife bg-nivayalife-mint/60 px-4 py-3 text-sm font-medium text-nivayalife-green dark:bg-nivayalife-green/15 dark:text-nivayalife-mint">
            ...
            <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div class="rounded-nivayalife bg-white p-5 shadow-nivayalife-sm dark:bg-white/5">
                    <h3 class="text-xs font-bold uppercase tracking-wide text-nivayalife-muted">BMI</h3>
                    @if($latestBmi)
                        <div class="mt-2">
                            <x-bmi-gauge :height-cm="$latestBmi->height_cm" :weight-kg="$latestBmi->weight_kg" :editable="false" :size="160" />
                        </div>
                        <p class="mt-1 text-center text-xs text-nivayalife-muted">Last recorded {{ $latestBmi->recorded_date->diffForHumans() }}</p>
                    @else
                        <p class="mt-4 text-center text-sm text-nivayalife-muted">No BMI recorded yet.</p>
                    @endif
                </div>

                <div class="space-y-4">
                    <div class="rounded-nivayalife bg-white p-5 shadow-nivayalife-sm dark:bg-white/5">
                        <h3 class="text-xs font-bold uppercase tracking-wide text-nivayalife-muted">Allergies</h3>
                        @if($allergies->isEmpty())
                            <p class="mt-2 text-sm text-nivayalife-muted">None recorded.</p>
                        @else
                            <div class="mt-2 flex flex-wrap gap-1.5">
                                @foreach($allergies as $allergen)
                                    <span class="rounded-full bg-nivayalife-pink/25 px-2.5 py-1 text-xs font-semibold text-nivayalife-pink-dark">{{ $allergen }}</span>
                                @endforeach
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="rounded-nivayalife bg-white p-5 shadow-nivayalife-sm dark:bg-white/5">
                        <h3 class="text-xs font-bold uppercase tracking-wide text-nivayalife-muted">Current medications</h3>
                        @if($medications->isEmpty())
                            <p class="mt-2 text-sm text-nivayalife-muted">None recorded.</p>
                        @else
                            <ul class="mt-2 space-y-1 text-sm text-nivayalife-ink dark:text-white">
            ...
            <div class="mt-4 flex justify-end">
                <a href="{{ route('family.member.edit', $member) }}" class="rounded-xl bg-nivayalife-green px-5 py-2.5 text-sm font-semibold text-white shadow-nivayalife-sm hover:bg-nivayalife-green-dark">Edit profile</a>
            </div>
        @endif
    </div>
</x-app-layout>
