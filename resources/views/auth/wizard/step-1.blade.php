@php
    $existing = $wizard['step1'] ?? [];
    $google = $wizard['google'] ?? null;
@endphp

<div x-ref="step1">
    <h2 class="text-xl font-bold text-novix-ink dark:text-white">Let's start with the basics</h2>
    <p class="mt-1 text-sm text-novix-muted">This becomes your login and your emergency ID card details.</p>

    <form x-ref="step1Form" class="mt-6 space-y-4" @submit.prevent>
        @if($google)
        ...
        <x-floating-input name="full_name" label="Full name" :value="$existing['full_name'] ?? ($google['name'] ?? '')" :required="true" autocomplete="name" dynamic-errors />

        @if($google)
            <input type="hidden" name="email" value="{{ $google['email'] }}">
        @else
            <div x-data="{ email: @js($existing['email'] ?? '') }" class="relative">
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <x-phone-input name="phone" label="Phone number" :value="$existing['phone'] ?? ''" dynamic-errors />
            <x-floating-input type="date" name="date_of_birth" label="Date of birth" :value="$existing['date_of_birth'] ?? ''" :required="true" dynamic-errors />
        </div>

        <x-floating-select name="gender" label="Gender" :required="true" dynamic-errors :value="$existing['gender'] ?? ''" :options="[
            'male' => 'Male', 'female' => 'Female', 'other' => 'Other', 'prefer_not_to_say' => 'Prefer not to say',
        ]" />

        <x-blood-group-select :value="$existing['blood_group'] ?? ''" dynamic-errors />
    </form>
</div>
