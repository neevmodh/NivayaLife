...
$wordColor = match (true) {
    $dark === 'responsive' => 'text-white sm:text-nivayalife-ink dark:sm:text-white',
    (bool) $dark => 'text-white',
    default => 'text-nivayalife-ink dark:text-white',
};
@endphp
...
<div {{ $attributes->merge(['class' => 'inline-flex items-center gap-2.5']) }}>
    ...
    <span class="{{ $s['tile'] }} flex flex-shrink-0 items-center justify-center bg-nivayalife-green shadow-nivayalife-sm">
    ...
    <span class="{{ $s['text'] }} font-bold tracking-tight {{ $wordColor }}">Nivaya<span class="text-nivayalife-gold"> Life</span></span>
</div>
