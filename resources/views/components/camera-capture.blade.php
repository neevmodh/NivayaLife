...
>
    <div class="relative mx-auto flex h-64 w-64 items-center justify-center overflow-hidden rounded-full bg-novix-ink/90 shadow-novix">
        <video x-ref="video" x-show="!hasCaptured && !cameraError" autoplay playsinline muted class="h-full w-full scale-x-[-1] object-cover"></video>

        <img x-show="hasCaptured && capturedDataUrl" :src="capturedDataUrl" class="h-full w-full object-cover" alt="Captured photo preview">

        <div x-show="!hasCaptured && !cameraError" x-cloak class="pointer-events-none absolute inset-4 rounded-full border-4 border-dashed border-white/50"></div>
    ...
    <div class="mt-5 flex flex-wrap items-center justify-center gap-3">
        <button x-show="!hasCaptured && !cameraError" x-cloak type="button" @click="capture()"
