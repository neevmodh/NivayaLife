...
@unless($hidden)
    ...
    <div x-data="{ open: false }" class="fixed bottom-24 right-4 z-40 flex flex-col items-end gap-2 sm:bottom-6 sm:right-6 sm:standalone:bottom-24"
        ...
        <div x-cloak x-show="open"
            ...
            class="w-56 origin-bottom-right rounded-nivayalife border-t-2 border-nivayalife-gold/50 bg-white p-3 shadow-nivayalife dark:bg-nivayalife-night">
            <p class="text-xs font-bold text-nivayalife-ink dark:text-white">Ask the assistant</p>
            <p class="mt-0.5 text-[11px] leading-relaxed text-nivayalife-muted">
                Questions about reports, medicines or how to use the app — by typing or voice.
            </p>
            <a href="{{ route('assistant') }}"
                class="mt-2.5 flex items-center justify-center gap-1.5 rounded-xl bg-nivayalife-green py-2 text-xs font-bold text-white transition hover:bg-nivayalife-green-dark active:scale-95">
                Open assistant
        ...
        <button type="button" @click="open = !open" @click.outside="open = false"
            class="group relative flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-nivayalife-green to-nivayalife-green-dark text-white shadow-nivayalife ring-2 ring-nivayalife-gold/50 transition hover:-translate-y-0.5 active:scale-95"
            ...
            <span class="absolute inline-flex h-full w-full animate-ping rounded-2xl bg-nivayalife-green/25 motion-reduce:hidden" aria-hidden="true" style="animation-duration:3s"></span>
